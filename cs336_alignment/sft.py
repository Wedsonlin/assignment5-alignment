...
if __name__ == "__main__":
    ...
    optim_steps_per_epoch = len(sft_data) // (microbatch_size * gradient_accumulation_steps)
    total_optim_steps = optim_steps_per_epoch * epoch_size
    ...
    if LOG:
        ...
        logger = Logger(project="sft", name=f"num_example_{label}{suffix}")
    ...
    for epoch in range(epoch_size):
        ...
        for i in range(0, input_ids.shape[0], microbatch_size):
            ...
            if micro_step % gradient_accumulation_steps == 0:
                ...
                if optim_step % eval_every_n_optim_steps == 0:
                    policy.eval()
                    with torch.no_grad():
    ...
    if SAVE_MODEL:
        save_label = "all" if sample_num == 0 else str(sample_num)
        save_suffix = "_filtered" if filtered else ""
        save_path = save_dir + f"num_example_{save_label}{save_suffix}"
